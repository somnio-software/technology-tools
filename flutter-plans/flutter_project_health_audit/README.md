...
### 🔄 Optional Best Practices Check

**User Prompt Required**: After completing the Flutter Project Health Audit (Step 9), the system will **prompt the user** to ask if they want to execute the Best Practices Check. **The Best Practices Check is NEVER executed automatically**.

**Execution Flow**:
1. Execute `@flutter-health.plan.md` completely (Steps 0-9)
2. After Step 9, **prompt the user** if they want to execute Best Practices Check
3. **Only if user confirms**, execute `@flutter_best_practices_check/plan/best_practices.plan.md`

**What Each Plan Provides**:
- **Health Audit** (`@flutter-health.plan.md`): Macro-level analysis
  - Project infrastructure, CI/CD, security, documentation
  - Test coverage, version management, repository structure
  - Output: `./reports/flutter_audit.txt`

- **Best Practices Check** (`@best_practices.plan.md`): Micro-level analysis
  - Code quality, testing standards, architecture compliance
  - Pattern recognition, granular inspection
  - Output: Generated by `@best_practices_generator.yaml`
