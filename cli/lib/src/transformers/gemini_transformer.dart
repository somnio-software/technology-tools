...
/// Result of transforming content for Gemini CLI.
class GeminiSkillOutput {
  const GeminiSkillOutput({
  ...
  /// The SKILL.md content with YAML frontmatter and transformed plan.
...
/// Transforms plan.md + YAML rules into Gemini CLI skill format.
///
/// Gemini CLI skills are stored as directories containing:
...
class GeminiTransformer {
  /// Transforms a skill bundle into Gemini CLI format.
  GeminiSkillOutput transform(SkillBundle bundle, ContentLoader loader) {
  ...
  String _generateSkillMd(SkillBundle bundle, String planContent) {
    // Gemini CLI uses name + description frontmatter.
    // The description includes "when to use" trigger text for on-demand
    // skill activation.
    final frontmatter = '---\n'
        'name: ${bundle.name}\n'
        'description: >-\n'
