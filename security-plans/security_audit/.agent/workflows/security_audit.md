...
4. **Gitleaks Scan (Optional)**
   Read `security_audit/cursor_rules/security_gitleaks.yaml` and execute the instructions to scan for secrets in git history. Skip gracefully if Gitleaks is not installed.
...
6. **Dependency Age Audit**
   Read `security_audit/cursor_rules/security_dependency_age.yaml` and execute the instructions to check outdated and deprecated dependencies.

7. **Trivy Scan (Optional)**
   Read `security_audit/cursor_rules/security_trivy.yaml` and execute the instructions to run Trivy filesystem scan. Skip if Trivy is not installed.

8. **SAST Scan**
   Read `security_audit/cursor_rules/security_sast.yaml` and execute the instructions to scan for OWASP patterns (SQL injection, XSS, path traversal).

9. **Gemini AI Analysis (Optional)**
